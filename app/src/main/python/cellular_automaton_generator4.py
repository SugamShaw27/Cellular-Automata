...
def generate_cellular_automaton(
    ...
    number_matrix = []
    non_uniform_rule_binary_list_matrix=[]
    ...
    for i in range(1, generations):
        ...
        non_uniform_rule_binary_list_matrix.append(list(non_uniform_rule_binary_list))

    return automaton, number_matrix,non_uniform_rule_binary_list_matrix
...
def rule_vector_matrix_save(vector):
    # vector_flat = np.concatenate(vector).reshape(-1, 1)
    ...
    plt.title(f"Rule Vector Matrix", pad=10, fontsize=12)
    plt.xlabel("Index", fontsize=12, labelpad=0)
    plt.ylabel("Rule", fontsize=12, labelpad=0)
    plt.xticks(
        np.arange(len(vector[0])) + 0.5, labels=range(len(vector[0])), rotation=90, fontsize=8,
        )
    plt.yticks(
        np.arange(len(vector)) + 0.5, labels=range(len(vector)), rotation=0, fontsize=8
    )
...
def generate_and_return_image(
    ...
    automaton, number_matrix,non_uniform_rule_binary_list_matrix = generate_cellular_automaton(
        automatoninitial,
        size=size,
        generations=generations,
        random=random_position,
    )
    print(non_uniform_rule_binary_list_matrix)
    ...
    rule_vector_matrix_img = rule_vector_matrix_save(non_uniform_rule_binary_list_matrix)
