...
def generate_cellular_automaton(
    ...
    for i in range(1, generations):
        new_state, matrix = generate_next_state(
            current_state, non_uniform_rule_binary_list, random
        )
...
def generate_and_return_image(
    ...
    current_state = generate_random_initial_state(size)
    automaton = np.zeros((generations, size), dtype=int)
    automaton[0] = current_state
...
# Example usage:
# img_str_automaton, img_str_combined = generate_and_return_image(15,20, False, "Title", "X Axis", "Y Axis", colormap='viridis')
# print(img_str_automaton,img_str_combined)